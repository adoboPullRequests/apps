...
class OC_USER_SAML_Hooks {
			...
			if ($usernameFound && $uid == $userid) {
				if ($samlBackend->updateUserData) {
					...
					update_user_data($uid, $attrs['email'], $attrs['groups'],
						$attrs['protected_groups'], $attrs['display_name'], $attrs['quota']);
				}
				return true;
			}
		}
	...
	static public function post_createUser($parameters) {
		...
		if (!$samlBackend->updateUserData) {
			...
			$attrs = get_user_attributes($uid, $samlBackend);
			update_user_data($uid, $attrs['email'], $attrs['groups'],
				$attrs['protected_groups'], $attrs['display_name'], $attrs['quota'], true);
		}
	}
...
function get_user_attributes($uid, $samlBackend) {
	...
	if (!empty($samlBackend->quotaMapping)) {
		...
		if (empty($result['quota']) && !empty($samlBackend->defaultQuota)) {
			...
			OC_Log::write('saml','Using default quota ('.$samlBackend->defaultQuota.') for user: '.$uid, OC_Log::DEBUG);
		}
		OC_Log::write('saml','User quota set: "'.$result['quota'].'" for: '.$uid, OC_Log::DEBUG);
	}
...
function update_user_data($uid, $email=null, $groups=null, $protectedGroups='', $displayName=null, $quota=null, $just_created=false) {
	OC_Util::setupFS($uid);
